...
namespace MyUI.models
{
    public class Service
    {
        ...
        public void scriptV2(string longviewpath, string solution, string config, int version)
        {
            const string name = "PATH";
            var platform="";
            var script = "";
            ...
            if (pathvar.ToLower().Contains("microsoft visual studio 10") == false && pathvar.ToLower().Contains("microsoft visual studio 12") == false)
            {
                pathvar = pathvar + ";" + @"C:\Program Files (x86)\Microsoft Visual Studio 12.0\Common7\IDE";
            }
            if (version > 7400) {
             d = Regex.Replace(pathvar, "Microsoft Visual Studio 10.0", "Microsoft Visual Studio 12.0");
             script = "Devenv " + @longviewpath + "\\" + solution + " /"+config+" \"Debug|"+platform+"\"";
            }
            else
            {
                 d = Regex.Replace(pathvar, "Microsoft Visual Studio 12.0", "Microsoft Visual Studio 10.0");
                 script = "Devenv " + @longviewpath + "\\" + solution + " /"+config +"\"Debug|" + platform + "\"";
            }
            System.Environment.SetEnvironmentVariable(name, d, target);
            File.WriteAllText(@"Build.ps1", script);
            File.WriteAllText(@"build.cake", "");
        }
    }
}
